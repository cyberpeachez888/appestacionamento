...
export default {
  ...
  const { data, error } = await supabase.from(table).insert(payload).select().single();
      ...
      await logEvent({ actor: req.user, action: 'monthlyCustomer.create', targetType: 'monthly_customer', targetId: id, details: { parkingSlot: payload.parking_slot } });
  ...
  async update(req, res) {
    try {
      ...
      if (b.plates !== undefined) patch.plates = Array.isArray(b.plates) ? JSON.stringify(b.plates) : b.plates; // accept already-string
      ...
      if (error) {
        ...
        if (error.code === '42703') { // undefined_column
          return res.status(500).json({
            ...
            message: 'Execute backend/COMPLETE-MIGRATION.sql (or MIGRATION-parking-slot.sql) in Supabase to add required columns (parking_slot, plates, cpf, phone, operator_name).',
            details: error.message,
            missingColumn: (error.message.match(/column \"(.+?)\"/) || [])[1]
          });
        }
        return res.status(400).json({ error: error.message || 'Failed to update monthly customer', details: error.details, hint: error.hint });
      }
      ...
      await logEvent({ actor: req.user, action: 'monthlyCustomer.update', targetType: 'monthly_customer', targetId: id, details: Object.keys(patch) });
  ...
  async remove(req, res) {
    ...
    await logEvent({ actor: req.user, action: 'monthlyCustomer.delete', targetType: 'monthly_customer', targetId: id });
    res.sendStatus(204);
  },
  ...
  async pay(req, res) {
    try {
      ...
      const payment = { id: uuid(), target_type: 'monthly_customer', target_id: id, date: new Date().toISOString(), value, method };
  ...
  const { data, error } = await supabase.from(table).update({ last_payment: payment.date, due_date: nextDue }).eq('id', id).select().single();
      if (error) return res.status(500).json({ error });
  await logEvent({ actor: req.user, action: 'monthlyCustomer.payment', targetType: 'monthly_customer', targetId: id, details: { value, method } });
