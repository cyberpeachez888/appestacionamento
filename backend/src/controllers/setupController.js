...
export const initialize = async (req, res) => {
  try {
    ...
    if (existingSettings?.id) {
      ...
      const result = await supabase.from('company_settings').insert(settingsData);
      companyError = result.error;
    }
    ...
    await supabase.from('users').delete().eq('login', 'admin');
    ...
    for (const vType of vehicleTypes) {
      await supabase.from('vehicle_types').insert(vType);
    }
