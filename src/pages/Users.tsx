...
const Users: React.FC = () => {
  ...
  const [bulkPending, setBulkPending] = useState<{ kind: 'role' | 'preset'; value: string } | null>(null);
  ...
  const [form, setForm] = useState({ name: '', email: '', login: '', password: '', role: 'operator', permissions: { ...defaultPermissions } });
  const [editForm, setEditForm] = useState({ name: '', email: '', login: '', role: 'operator', permissions: { ...defaultPermissions } });
  ...
  type Strength = { score: number; label: 'Muito fraca' | 'Fraca' | 'Média' | 'Forte' | 'Muito forte'; color: string; valid: boolean; issues: string[] };
  const evaluatePassword = (pwd: string): Strength => {
    ...
    const colors = ['text-red-600','text-orange-500','text-yellow-600','text-green-600','text-emerald-600'];
  ...
  const openCreate = () => {
    setForm({ name: '', email: '', login: '', password: '', role: 'operator', permissions: { ...defaultPermissions } });
    setCreateOpen(true);
  };
  ...
  const submitCreate = async (e: React.FormEvent) => {
    ...
    if (!createPwdStrength.valid) {
      toast({ title: 'Senha fraca', description: createPwdStrength.issues.join(' • '), variant: 'destructive' });
      return;
    }
    try {
  ...
  const openEdit = (u: any) => {
    setSelectedUser(u);
    setEditForm({ name: u.name, email: u.email, login: u.login, role: u.role, permissions: { ...defaultPermissions, ...(u.permissions || {}) } });
    setEditOpen(true);
  };
  ...
  const submitPassword = async (e: React.FormEvent) => {
    ...
    if (!newPwdStrength.valid) {
      toast({ title: 'Senha fraca', description: newPwdStrength.issues.join(' • '), variant: 'destructive' });
      return;
    }
    try {
      if (oldPassword) {
  ...
  const selectedAdminsCount = useMemo(() => users.filter(u => selectedIds.includes(u.id) && u.role === 'admin').length, [users, selectedIds]);
  ...
  const bulkSetRole = async (role: 'admin' | 'operator') => {
    ...
    if (role === 'operator') {
      ...
      if (countAdmins > 0 && selectedAdminsCount === countAdmins) {
        toast({ title: 'Operação bloqueada', description: 'Não é permitido remover o último admin.', variant: 'destructive' });
        return;
      }
    }
    try {
      ...
      toast({ title: 'Papel atualizado', description: `${selectedIds.length} usuário(s) atualizado(s)` });
      setSelectedIds([]);
      await loadUsers();
    } catch (err: any) {
      toast({ title: 'Erro em atualização em massa', description: err.message, variant: 'destructive' });
    }
  };
  ...
  const bulkApplyPreset = async (name: keyof typeof PRESETS) => {
    ...
    const p = PRESETS[name];
    try {
      const ops = selectedIds.map(id => api.updateUser(id, { role: p.role, permissions: p.permissions }));
      await Promise.all(ops);
      toast({ title: 'Preset aplicado', description: `${name} aplicado para ${selectedIds.length} usuário(s)` });
      setSelectedIds([]);
      await loadUsers();
    } catch (err: any) {
  ...
  const exportJSON = () => {
    const data = filteredUsers.map(u => ({
      ...
      email: u.email,
      role: u.role,
      permissions: permKeys.reduce((acc: any, k) => ({ ...acc, [k]: Boolean((u.permissions || {})[k]) }), {})
    }));
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6 gap-3">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={() => setAuditLogOpen(true)}>Log de Auditoria</Button>
        ...
        <div className="bg-card rounded-lg border p-4 mb-4">
              ...
              <Select value={permFilter} onValueChange={(v) => setPermFilter(v as any)}>
                ...
                <SelectContent>
                  ...
                  {permissionKeys.map(k => (
                    <SelectItem key={k} value={k}>{permissionLabel(k)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          {(query || roleFilter !== 'all' || permFilter !== 'any') && (
            <div className="flex items-center justify-between mt-3 text-sm text-muted-foreground">
              ...
              <Button variant="ghost" size="sm" onClick={() => { setQuery(''); setRoleFilter('all'); setPermFilter('any'); }}>Limpar filtros</Button>
            </div>
          )}
        </div>
        ...
        {selectedIds.length > 0 && (
          <div className="bg-amber-50 border border-amber-200 text-amber-900 rounded-md p-3 mb-3 flex flex-wrap items-center gap-2">
            ...
            <div className="ml-auto flex items-center gap-2">
              <Select onValueChange={(v) => { setBulkPending({ kind: 'role', value: v }); setBulkOpen(true); }}>
                <SelectTrigger className="w-[170px]"><SelectValue placeholder="Definir papel" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="admin">Admin</SelectItem>
                  <SelectItem value="operator">Operador</SelectItem>
                </SelectContent>
              </Select>
              <Select onValueChange={(v) => { setBulkPending({ kind: 'preset', value: v }); setBulkOpen(true); }}>
                <SelectTrigger className="w-[200px]"><SelectValue placeholder="Aplicar preset" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="Admin">Admin</SelectItem>
                  ...
                  <td className="px-4 py-3 text-right flex gap-1 justify-end">
                    {hasPermission('manageUsers') && (
                      <>
                        ...
                        <Button variant="ghost" size="sm" onClick={() => openPassword(u)}>Senha</Button>
                        <Button variant="destructive" size="sm" onClick={() => openDelete(u)}>Excluir</Button>
                      </>
                    )}
                  </td>
                </tr>
            ...
            <form onSubmit={submitCreate} className="space-y-3">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  <Button type="button" variant="outline" size="sm" onClick={() => applyCreatePreset('Admin')}>Admin</Button>
                  <Button type="button" variant="outline" size="sm" onClick={() => applyCreatePreset('Operacional')}>Operacional</Button>
                  <Button type="button" variant="outline" size="sm" onClick={() => applyCreatePreset('Financeiro')}>Financeiro</Button>
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <Input type={showPasswords ? 'text' : 'password'} value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} required />
                {form.password && (
                ...
                {!createPwdStrength.valid && form.password && (
                  <p className="text-xs text-muted-foreground">{createPwdStrength.issues.join(' • ')}</p>
                )}
              </div>
              <div>
                <label className="text-sm font-medium">Papel</label>
                <Select value={form.role} onValueChange={(val) => setForm({ ...form, role: val as 'admin' | 'operator', permissions: val === 'admin' ? { ...allPermissionsTrue } : { ...defaultPermissions } })}>
                ...
                {Object.keys(defaultPermissions).map((key) => (
                  <label key={key} className="flex items-center gap-2 text-sm">
                    <Checkbox
                      ...
                      onCheckedChange={(v) => setForm({ ...form, permissions: { ...form.permissions, [key]: Boolean(v) } })}
                    />
          ...
          <DialogContent>
            ...
            <form onSubmit={submitEdit} className="space-y-3">
              <div>
                <div className="text-xs text-muted-foreground mb-2">Presets rápidos</div>
                <div className="flex flex-wrap gap-2">
                  <Button type="button" variant="outline" size="sm" onClick={() => applyEditPreset('Admin')}>Admin</Button>
                  <Button type="button" variant="outline" size="sm" onClick={() => applyEditPreset('Operacional')}>Operacional</Button>
                  <Button type="button" variant="outline" size="sm" onClick={() => applyEditPreset('Financeiro')}>Financeiro</Button>
                </div>
              </div>
              <div>
                <label className="text-sm font-medium">Nome</label>
              ...
              <div>
                <label className="text-sm font-medium">Login</label>
                <Input value={editForm.login} onChange={e => setEditForm({ ...editForm, login: e.target.value })} required />
              </div>
              <div>
                <label className="text-sm font-medium">Papel</label>
                <Select value={editForm.role} onValueChange={(val) => setEditForm({ ...editForm, role: val as 'admin' | 'operator', permissions: val === 'admin' ? { ...allPermissionsTrue } : { ...editForm.permissions } })}>
                  <SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="operator">Operador</SelectItem>
                    <SelectItem value="admin">Admin</SelectItem>
              ...
              {editForm.role === 'admin' ? (
                <div className="text-xs text-muted-foreground">Admins possuem acesso total por padrão.</div>
              ) : (
                <div className="space-y-2">
                  <div className="text-sm font-medium">Permissões</div>
                  {Object.keys(defaultPermissions).map((key) => (
                    <label key={key} className="flex items-center gap-2 text-sm">
                      <Checkbox
                        ...
                        onCheckedChange={(v) => setEditForm({ ...editForm, permissions: { ...editForm.permissions, [key]: Boolean(v) } })}
                      />
                      <span className="inline-flex items-center gap-1">
                        {permissionLabel(key)}
                        <Tooltip>
          ...
          <DialogContent>
            ...
            <form onSubmit={submitPassword} className="space-y-3">
              <div className="text-xs text-muted-foreground">
                Administradores podem redefinir senhas sem informar a senha atual. Usuários comuns devem informar a senha atual.
              </div>
              <div>
                <label className="text-sm font-medium">Senha atual (somente se for sua própria conta)</label>
                <Input
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  {!newPwdStrength.valid && newPassword && (
                    <p className="text-xs text-muted-foreground">{newPwdStrength.issues.join(' • ')}</p>
                  )}
                </div>
                <div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="h-4 w-4"
                />
                <label htmlFor="showPasswords" className="text-sm cursor-pointer select-none">Mostrar senhas</label>
              </div>
              <DialogFooter>
        ...
        <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
          <DialogContent>
            ...
            <p>Tem certeza que deseja excluir {selectedUser?.name}? Esta ação não pode ser desfeita.</p>
            <DialogFooter>
        ...
        <Dialog open={bulkOpen} onOpenChange={setBulkOpen}>
          <DialogContent>
            ...
            <div className="space-y-2 text-sm">
              <p>
                Você está prestes a aplicar <span className="font-medium">{bulkPending?.kind === 'role' ? `o papel "${bulkPending?.value}"` : `o preset "${bulkPending?.value}"`}</span> em
                {' '}<span className="font-medium">{selectedIds.length}</span> usuário(s).
              </p>
              {bulkPending?.kind === 'role' && bulkPending?.value === 'operator' && selectedAdminsCount === countAdmins && countAdmins > 0 && (
                <p className="text-red-600">Atenção: isso removeria o último admin, o que não é permitido.</p>
              )}
            </div>
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => { setBulkOpen(false); setBulkPending(null); }}>Cancelar</Button>
...
function permissionHelp(key: string) {
  const map: Record<string, string> = {
    manageRates: 'Permite criar, editar e remover tarifas. Usuários sem esta permissão só conseguem visualizar as tarifas.',
    manageMonthlyCustomers: 'Permite cadastrar, editar e excluir mensalistas, bem como registrar pagamentos de planos mensais.',
    viewReports: 'Permite acessar relatórios financeiros e operacionais. Sem esta permissão, a página de relatórios fica inacessível.',
    manageUsers: 'Permite criar, editar, redefinir senha e excluir usuários, além de gerenciar papéis e permissões.',
    manageCompanyConfig: 'Permite alterar configurações gerais da empresa (nome, CNPJ, recibos, etc.). Sem esta permissão o formulário fica somente leitura.',
    manageVehicleTypes: 'Permite gerenciar os tipos de veículos disponíveis (criar, editar, remover).',
    openCloseCash: 'Permite abrir e fechar o caixa, além de registrar entradas/saídas avulsas e finalizar tickets.',
    manageBackups: 'Permite criar, baixar, restaurar e excluir backups do sistema, bem como configurar backups automáticos programados.',
  };
