...
export default function HorariosFeriados() {
  ...
  const fetchBusinessHours = async () => {
    try {
      ...
      if (data.success) {
        ...
        toast({
          title: 'Erro',
          description: data.error || 'Falha ao carregar horários',
          variant: 'destructive',
        });
      }
    } catch (error) {
  ...
  const handleSaveHoliday = async (holiday: Partial<Holiday>) => {
    try {
      ...
      const url = holiday.id
        ? `${API_BASE_URL}/holidays/${holiday.id}`
  ...
  const handleSaveEvent = async (event: Partial<SpecialEvent>) => {
    try {
      const token = getAuthToken();
      const url = event.id
        ? `${API_BASE_URL}/special-events/${event.id}`
    ...
    <div className="p-6 space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
                ...
                businessHours.map((hour) => {
                  ...
                  return (
                    <div key={hour.id} className="flex justify-between p-4 border rounded mb-2">
                      ...
                      <Button size="icon" variant="ghost" onClick={() => handleEditHour(hour)}>
        ...
        <TabsContent value="feriados">
          <Button onClick={handleAddHoliday} className="mb-4">
                ...
                <TableBody>
                  {holidays.map((h) => (
                    <TableRow key={h.id}>
                      ...
                      <TableCell>
                        <Button size="icon" variant="ghost" onClick={() => handleEditHoliday(h)}>
                          <Pencil className="w-4 h-4" />
                        </Button>
                        <Button
                          size="icon"
                ...
                <TableBody>
                  {specialEvents.map((e) => (
                    <TableRow key={e.id}>
                      ...
                      <TableCell>
                        <Button size="icon" variant="ghost" onClick={() => handleEditEvent(e)}>
                          <Pencil className="w-4 h-4" />
                        </Button>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Editar - {selectedHour ? getDayName(selectedHour.day_of_week) : ''}
            </DialogTitle>
          </DialogHeader>
          {selectedHour && (
            <div className="space-y-4">
              <div className="flex items-center space-x-2">
                <Switch
                  checked={selectedHour.is_open}
                  onCheckedChange={(c) => setSelectedHour({ ...selectedHour, is_open: c })}
                />
                <Label>Aberto</Label>
              </div>
              {selectedHour.is_open && (
                <>
                  <div className="space-y-2">
                    ...
                    <Input
                      type="time"
                      value={selectedHour.open_time || ''}
                      onChange={(e) =>
                        setSelectedHour({ ...selectedHour, open_time: e.target.value })
                      }
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      type="time"
                      value={selectedHour.close_time || ''}
                      onChange={(e) =>
                        setSelectedHour({ ...selectedHour, close_time: e.target.value })
                      }
                    />
                  </div>
                </>
      ...
      <Dialog open={holidayDialogOpen} onOpenChange={setHolidayDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{selectedHoliday?.id ? 'Editar' : 'Adicionar'} Feriado</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>Nome</Label>
              <Input
                ...
                onChange={(e) =>
                  setSelectedHoliday({ ...selectedHoliday, name: e.target.value } as Holiday)
                }
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setSelectedHoliday({ ...selectedHoliday, date: e.target.value } as Holiday)
                }
              />
            </div>
            <div className="flex items-center space-x-2">
              <Switch
                checked={selectedHoliday?.is_recurring || false}
                onCheckedChange={(c) =>
                  setSelectedHoliday({ ...selectedHoliday, is_recurring: c } as Holiday)
                }
              />
            ...
            <div className="flex items-center space-x-2">
              <Switch
                checked={selectedHoliday?.is_closed || false}
                onCheckedChange={(c) =>
                  setSelectedHoliday({ ...selectedHoliday, is_closed: c } as Holiday)
                }
              />
              <Label>Fechado</Label>
            </div>
      ...
      <Dialog open={eventDialogOpen} onOpenChange={setEventDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{selectedEvent?.id ? 'Editar' : 'Adicionar'} Evento</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>Nome</Label>
              <Input
                ...
                onChange={(e) =>
                  setSelectedEvent({ ...selectedEvent, name: e.target.value } as SpecialEvent)
                }
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setSelectedEvent({ ...selectedEvent, start_date: e.target.value } as SpecialEvent)
                }
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setSelectedEvent({ ...selectedEvent, end_date: e.target.value } as SpecialEvent)
                }
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                type="number"
                step="0.1"
                value={selectedEvent?.price_multiplier || 1}
                onChange={(e) =>
                  setSelectedEvent({
                    ...selectedEvent,
